...
/*定义多播常量*/
//224.0.0.0 到 224.0.0.255 被 IANA 预留，地址 224.0.0.0 保留不做分配
#define MCASTADDR     "224.8.8.8" //在多播地址中是224 - 239 其中224.0.0.1以及几个是预设的不能用，其他的随便用
#define MCASTPORT     7777 //端口号
#define BUFSIZE       1024 //数据大小
#define MCOUNT        10 //消息数量

/*定义广播常量*/
#define BCASTPORT     5050 //端口
#define BCOUNT        10 //次数

/*定义广播全局变量*/
SOCKET             socketBro; //广播发送套接字
SOCKET             socketRec; //广播接收套接字
struct sockaddr_in addrBro; //广播发送地址
struct sockaddr_in addrRec;//广播接收地址
BOOL               broadSendFlag; //广播发送是否成功
BOOL               broadFlag; //广播接收是否成功

DWORD              bCount; //广播发送次数
DWORD              bcastAddr; //广播地址
short              bPort;//广播端口

/*定义多播全局变量*/
SOCKET             socketMul; //发送多播数据
SOCKET             sockJoin;//加入多播组
struct sockaddr_in addrLocal;//本地地址
struct sockaddr_in addrMul;//多播组地址

BOOL               multiSendFlag; //是否在进行多播发送
BOOL               bLoopBack;    //是否允许回送
BOOL               multiFlag; //是否启用多播

DWORD              dwInterface;  //接口标识
DWORD              dwMulticastGroup;//多播组地址
DWORD              mCount;         //计数器
short              mPort;           //多播端口
...
void broadcastSend(char *msg);
void broadcastRec();

void mulControl();
void multicastSend(char *msg);
void multicastRec();
            ...
            if (argv[i][0] == '-')
    		{
                switch (tolower(argv[i][1]))
        		{
                	...
                	case 's': 
            			...
            			break;
                    /*广播的地址*/
                	case 'h': //这个检测需要-h*192. 之类的输入，也就是如果是-h 192.地址，就不能正确识别
                       /* if (strlen(argv[i]) > 3)
                            bcastAddr = inet_addr(&argv[i][3]);*/
                        i++;
                        if(argv[i][0] >= '1' && argv[i][0] <= '9')
                        {
                            bcastAddr = inet_addr(&argv[i][0]);
                        }
            			break;
                    /*广播的端口号*/
                	case 'p':
...
void broadcastSend(char *msg)
{
    /*设置广播的消息*/
    if(msg == NULL)
        msg="The message received is from sender!";
    BOOL opt=TRUE;
...
int main(int argc, char **argv)
{
    ...
    {
        if(argv[i][0] == '-' && argv[i][1] == 'r' && strlen(argv[i]) > 3)
        {
            buf = &argv[i][2];
            break;
        }
