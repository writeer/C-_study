...
static void AddItemEqal(const Item * new, Trnode * desc);//复制到对应数组 如果是相同项，就添加到同一个节点
...
static void AddItemEqal(const Item * new, Trnode * desc)
{
    strcpy(desc->item.petkind[desc->item.index],new->petkind[0]);
}

Item * InTree(const Item * pi,const Tree * ptree)
{
    Trnode * current;
    if(((current = SeekItem(pi,ptree).child) != NULL) ? false : true)
        return NULL;
    else
        return &(current->item);
}
...
static bool ToRight(const Item * i1,const Item * i2)
{
    ...
    if((comp1 = strcmp(i1->petname, i2->petname)) > 0)// 只比较名
        return true;
    /*else if (comp1 == 0 && strcmp(i1->petkind,i2->petkind) > 0)
        return true;*/
    else
